...
export const DCAStrategyDef: StrategyDefinition = {
  ...
  description: 'Buy fixed USDC amount at regular intervals, regardless of price. Reduces timing risk.',
...
export const GridTradingStrategyDef: StrategyDefinition = {
  name: 'grid_trading',
  label: 'Grid Trading',
  description: 'Automatically buy at support levels and sell at resistance. Works best in sideways markets.',
  supportedIntents: ['swap'],
  paramSchema: z.object({
...
export const MomentumTradingStrategyDef: StrategyDefinition = {
  ...
  description: 'Buy tokens with positive price momentum, sell on reversal. Rides uptrends and downtrends.',
  supportedIntents: ['swap'],
  paramSchema: z.object({
    baseToken: z.string().default('USDC'),
...
export const ArbitrageStrategyDef: StrategyDefinition = {
  name: 'arbitrage',
  label: 'Arbitrage',
  description: 'Monitor price differences across DEXes (Jupiter, Raydium, Orca) and trade the spread.',
  supportedIntents: ['swap'],
  paramSchema: z.object({
    baseToken: z.string().default('USDC'),
    targetToken: z.string().default('SOL'),
...
export const AirdropFarmerStrategyDef: StrategyDefinition = {
  ...
  description: 'Track on-chain governance tokens and claim eligible airdrops. Realistic airdrop farming.',
  supportedIntents: ['harvest_yield', 'swap'],
  paramSchema: z.object({
...
const strategyFieldDescriptors: Record<string, SharedStrategyFieldDescriptor[]> = {
  dca: [
    { key: 'buyAmount', label: 'Buy Amount (USDC)', type: 'number', description: 'Amount to buy each cycle', required: false, default: 100 },
    { key: 'buyToken', label: 'Buy Token', type: 'string', description: 'Token to spend (USDC, USDT)', required: false, default: 'USDC' },
    { key: 'targetToken', label: 'Target Token', type: 'string', description: 'Token to accumulate', required: false, default: 'SOL' },
    { key: 'swapDex', label: 'DEX', type: 'string', description: 'Exchange to use', required: false, default: 'jupiter' },
    { key: 'maxSlippage', label: 'Max Slippage (%)', type: 'number', description: 'Acceptable price slippage', required: false, default: 0.5 },
    { key: 'frequencyHours', label: 'Buy Frequency (Hours)', type: 'number', description: 'How often to buy', required: false, default: 24 },
    { key: 'maxBuysPerDay', label: 'Max Buys / Day', type: 'number', description: 'Daily buy limit', required: false, default: 2 },
  ],
  grid_trading: [
    { key: 'baseToken', label: 'Base Token', type: 'string', description: 'Token to use for trading (USDC)', required: false, default: 'USDC' },
    { key: 'targetToken', label: 'Target Token', type: 'string', description: 'Token to trade', required: false, default: 'SOL' },
    { key: 'gridLevels', label: 'Grid Levels', type: 'number', description: 'Number of price levels', required: false, default: 10 },
    { key: 'gridSpread', label: 'Grid Spread (%)', type: 'number', description: 'Spread between grid levels', required: false, default: 2 },
    { key: 'buyAmount', label: 'Buy Amount per Level', type: 'number', description: 'USDC per grid level', required: false, default: 100 },
    { key: 'swapDex', label: 'DEX', type: 'string', description: 'Exchange to use', required: false, default: 'jupiter' },
    { key: 'maxSlippage', label: 'Max Slippage (%)', type: 'number', description: 'Acceptable price slippage', required: false, default: 0.5 },
    { key: 'priceCheckIntervalMins', label: 'Check Interval (Mins)', type: 'number', description: 'How often to check prices', required: false, default: 30 },
  ],
  momentum_trading: [
    { key: 'baseToken', label: 'Base Token', type: 'string', description: 'Token to use for trading (USDC)', required: false, default: 'USDC' },
    { key: 'trackingTokens', label: 'Track Tokens', type: 'string[]', description: 'Tokens to monitor', required: false, default: ['SOL', 'JUP'] },
    { key: 'lookbackPeriodHours', label: 'Lookback Period (Hours)', type: 'number', description: 'Historical period for momentum', required: false, default: 24 },
    { key: 'momentumThreshold', label: 'Momentum Threshold (%)', type: 'number', description: 'Min % gain to trigger buy', required: false, default: 5 },
    { key: 'investmentPerToken', label: 'Investment per Token', type: 'number', description: 'USDC to invest per token', required: false, default: 50 },
    { key: 'takeProfit', label: 'Take Profit (%)', type: 'number', description: 'Sell profit target', required: false, default: 10 },
    { key: 'stopLoss', label: 'Stop Loss (%)', type: 'number', description: 'Loss limit to exit', required: false, default: 5 },
  ],
  arbitrage: [
    { key: 'baseToken', label: 'Base Token', type: 'string', description: 'Trading pair (e.g., USDC)', required: false, default: 'USDC' },
    { key: 'targetToken', label: 'Target Token', type: 'string', description: 'Trading pair (e.g., SOL)', required: false, default: 'SOL' },
    { key: 'minimumSpread', label: 'Min Spread (%)', type: 'number', description: 'Minimum profitable spread', required: false, default: 2 },
    { key: 'tradeSize', label: 'Trade Size (USDC)', type: 'number', description: 'Amount per arbitrage trade', required: false, default: 500 },
    { key: 'maxSlippage', label: 'Max Slippage (%)', type: 'number', description: 'Acceptable price slippage', required: false, default: 0.3 },
    { key: 'priceCheckIntervalSecs', label: 'Check Interval (Secs)', type: 'number', description: 'How often to check prices', required: false, default: 60 },
    { key: 'maxTradesPerHour', label: 'Max Trades / Hour', type: 'number', description: 'Hourly trade limit', required: false, default: 10 },
  ],
  stop_loss_guard: [
    { key: 'baseToken', label: 'Base Token', type: 'string', description: 'Token for trading (USDC)', required: false, default: 'USDC' },
    { key: 'monitoredToken', label: 'Monitored Token', type: 'string', description: 'Token to protect', required: false, default: 'SOL' },
    { key: 'stopLossPercent', label: 'Stop Loss (%)', type: 'number', description: 'Sell if price drops this much', required: false, default: 10 },
    { key: 'checkIntervalMins', label: 'Check Interval (Mins)', type: 'number', description: 'How often to check price', required: false, default: 15 },
    { key: 'swapDex', label: 'DEX', type: 'string', description: 'Exchange to use', required: false, default: 'jupiter' },
    { key: 'maxSlippage', label: 'Max Slippage (%)', type: 'number', description: 'Acceptable price slippage', required: false, default: 0.7 },
  ],
  yield_harvesting: [
    { key: 'stakedTokens', label: 'Staked Tokens', type: 'string[]', description: 'Tokens to harvest from', required: false, default: ['mSOL'] },
    { key: 'autoCompound', label: 'Auto Compound', type: 'boolean', description: 'Reinvest rewards', required: false, default: true },
    { key: 'compoundThreshold', label: 'Compound Threshold (USD)', type: 'number', description: 'Min value to compound', required: false, default: 10 },
    { key: 'swapToToken', label: 'Swap Rewards To', type: 'string', description: 'Token to convert rewards to', required: false, default: 'SOL' },
    { key: 'swapDex', label: 'DEX', type: 'string', description: 'Exchange to use', required: false, default: 'jupiter' },
    { key: 'maxSlippage', label: 'Max Slippage (%)', type: 'number', description: 'Acceptable price slippage', required: false, default: 0.3 },
    { key: 'harvestFrequencyHours', label: 'Harvest Frequency (Hours)', type: 'number', description: 'How often to harvest', required: false, default: 12 },
  ],
  portfolio_rebalancer: [
    { key: 'rebalanceThreshold', label: 'Rebalance Threshold (%)', type: 'number', description: 'Drift before rebalancing', required: false, default: 5 },
    { key: 'swapDex', label: 'DEX', type: 'string', description: 'Exchange to use', required: false, default: 'jupiter' },
    { key: 'maxSlippage', label: 'Max Slippage (%)', type: 'number', description: 'Acceptable price slippage', required: false, default: 0.5 },
    { key: 'rebalanceFrequencyHours', label: 'Rebalance Frequency (Hours)', type: 'number', description: 'Max time between rebalances', required: false, default: 168 },
  ],
  airdrop_farmer: [
    { key: 'trackedTokens', label: 'Track Tokens', type: 'string[]', description: 'Governance tokens to monitor', required: false, default: ['JUP', 'ORCA'] },
    { key: 'autoClaimEligible', label: 'Auto Claim', type: 'boolean', description: 'Automatically claim eligible airdrops', required: false, default: true },
    { key: 'minAirdropValue', label: 'Min Airdrop Value (USD)', type: 'number', description: 'Minimum value to claim', required: false, default: 50 },
    { key: 'sellAirdropImmediately', label: 'Sell Immediately', type: 'boolean', description: 'Sell airdrop tokens right away', required: false, default: false },
    { key: 'swapDex', label: 'DEX', type: 'string', description: 'Exchange to use', required: false, default: 'jupiter' },
    { key: 'maxSlippage', label: 'Max Slippage (%)', type: 'number', description: 'Acceptable price slippage', required: false, default: 1 },
    { key: 'checkFrequencyHours', label: 'Check Frequency (Hours)', type: 'number', description: 'How often to check for airdrops', required: false, default: 6 },
  ],
};
